	...
	<div class="canvas-wrapper">
		<div class="popup-header">
			<div class="d-flex align-items-center justify-content-between mb-12">
				<h5 class="title"><?php esc_html_e( 'Shopping Cart', 'noorifa' ); ?></h5>
				<span class="icon-close-popup" data-bs-dismiss="offcanvas"><?php \Noorifa\Setup\Icons::render( 'X2' ); ?></span>
			</div>
			...
			<?php if ( $free_shipping_threshold && ! $cart->is_empty() ) : ?>
				...
				<div class="cart-threshold">
					...
					<?php else : ?>
						<p class="text"><?php esc_html_e( "You've unlocked free shipping!", 'noorifa' ); ?></p>
					<?php endif; ?>
					<div class="progress-bar progress-ship">
						<div class="value" style="width: <?php echo esc_attr( (string) $percent ); ?>%">
							<?php \Noorifa\Setup\Icons::render( 'Truck' ); ?>
						</div>
					</div>
				</div>
			<?php endif; ?>
							...
							<?php if ( $cart->is_empty() ) : ?>
								<div class="box-text_empty type-shop_cart">
									<div class="shop-empty_top">
										...
										<h4 class="text-emp"><?php esc_html_e( 'Your cart is empty', 'noorifa' ); ?></h4>
										<p class="cl-text-2"><?php esc_html_e( 'Your cart is currently empty. Let us help you find the right product.', 'noorifa' ); ?></p>
									</div>
									<div class="shop-empty_bot">
										<?php if ( function_exists( 'wc_get_page_permalink' ) ) : ?>
											<a href="<?php echo esc_url( wc_get_page_permalink( 'shop' ) ); ?>" class="btn animate-btn"><?php esc_html_e( 'Shopping', 'noorifa' ); ?></a>
										<?php endif; ?>
										<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="btn btn-stroke"><?php esc_html_e( 'Back to home', 'noorifa' ); ?></a>
									</div>
								</div>
							<?php else : ?>
									...
									<div class="mini-cart-item">
										...
										<div class="mini-cart-info">
											<a href="<?php echo esc_url( $product->get_permalink( $cart_item ) ); ?>" class="name fw-medium link text-line-clamp-1">
												<?php echo wp_kses_post( \Noorifa\WooCommerce\CartItemVariations::get_title( $cart_item ) ); ?>
											</a>
										...
										<div class="mini-cart-price">
											<a
												...
												aria-label="<?php echo esc_attr( sprintf( __( 'Remove %s from cart', 'noorifa' ), wp_strip_all_tags( \Noorifa\WooCommerce\CartItemVariations::get_title( $cart_item ) ) ) ); ?>"
												data-product_id="<?php echo esc_attr( (string) $product->get_id() ); ?>"
												data-cart_item_key="<?php echo esc_attr( $cart_item_key ); ?>"
												data-product_sku="<?php echo esc_attr( $product->get_sku() ); ?>"
											>
												<span class="text-caption-01 fw-semibold"><?php esc_html_e( 'Remove', 'noorifa' ); ?></span>
											</a>
					...
					<div class="mini-cart-bottom-wrap">
						<div class="mini-cart-total">
							<h5 class="text-total d-flex align-content-center justify-content-between">
								<span class="subtotal"><?php esc_html_e( 'Subtotal', 'noorifa' ); ?></span>
						...
						<a href="<?php echo esc_url( wc_get_page_permalink( 'shop' ) ); ?>" class="d-flex justify-content-center fw-semibold text-center link" data-bs-dismiss="offcanvas">
							<?php esc_html_e( 'Or Continue Shopping', 'noorifa' ); ?>
						</a>
					</div>
				<?php endif; ?>
			</div>
